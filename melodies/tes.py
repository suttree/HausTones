...
time += 1.0 + random.uniform(0.5, 2.2)
...
for i in range(iterations):
    ...
    if i % 2 == 0 and i > 0:
      for j, note in enumerate(notes[::-1]):
          timeline.add(time+1.075*j+1, Hit(Note(note).shift_down_octave(1), duration))
      time += duration

    # Ascending & desending xover
    for j, note in enumerate(notes):
      timeline.add(time + 0.25*j*2, Hit(Note(note).shift_down_octave(1), duration))
    # Descending arppegio
    #for j, note in enumerate(notes[::-1]):
    #  timeline.add(time + 0.25*j*2, Hit(Note(note).shift_down_octave(1), duration))
    time += duration
    ...
    for n in range(8):
      for j, note in enumerate(notes[::-1]):
          #timeline.add(time + offset * j, Hit(Note(note).shift_down_octave(1), duration))
          timeline.add(duration/4 + time + offset * j, Hit(Note(note).shift_down_octave(1), duration))
      time += duration/2
...
# Cavernous ascender
for j, note in enumerate(notes):
    timeline.add(time+0.95*j, Hit(Note(note).shift_down_octave(1), duration))

print("Rendering audio...")
data = timeline.render(8)
#data = effect.shimmer(data, 0.24)
#data = effect.tremolo(data, 0.1)
data = effect.feedback_modulated_delay(data, data, 1.0, 0.24)
