...
time = 0.0
offset = 0.0
iterations = random.randint(18, 52)
duration = 4.0
...
def triad(time):
  timeline.add(time + 0.042 + math.sin(increment), Hit(Note(notes[0]), duration))
  timeline.add(time + 0.083 + math.sin(increment), Hit(Note(notes[3]), duration))
  timeline.add(time + 0.094 + math.cos(increment), Hit(Note(notes[5]), duration))
      
def strum(time, offset = 0.065):
...
for i in range(iterations):
  ...
  time += quarter_note + math.sin(time)
  triad(time)
