...
# Function to play a chord using the sounddevice module

def play_chord(chord_frequencies, duration, fs):
    """
    This function generates a chord from the given frequencies and plays it for the given duration.

    :param chord_frequencies: list of frequencies that make up the chord
    :param duration: duration to play the chord in seconds
    ...
    # Generate the samples for each frequency
    chord_samples = sum(np.sin(2 * np.pi * np.arange(samples) * f / fs) for f in chord_frequencies)

    # Normalize the amplitude of the chord to avoid clipping
    chord_samples /= len(chord_frequencies)

    # Play the chord
    sd.play(chord_samples, fs)
    sd.wait()  # Wait until the chord is done playing
